...
function OffersList(props: OffersListExtendedProps): JSX.Element {
  ...
  return (
    <div className={`${className} places__list`}>
      {offers.map((offer) => (<OfferCard offers={offers} cardClassName={cardClassName} key={offer.id} id={offer.id} onActivate={() => setActive(offer.id)} onDeactivate={() => setActive(undefined)} />))}
    </div>
  );
}
